...
%Training neural network means finding weights and biases. Process of finding weights and biases is 'training neural network'. This part shows how to train neural network by backpropagation.
%Let $\vec{x} \in \mathbb{R}^n$ be input, $\vec{y} \in \mathbb{R}^m$ be label, $\vec{b} \in \mathbb{R}^m$ be bias, and  $\bold{w}: \mathbb{R}^n \rightarrow \mathbb{R}^m$ be transform matrix.
